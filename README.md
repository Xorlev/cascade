# Cascade: Structured Log Daemon

Cascade is a TCP-accessible, low-resource, Kafka-like log daemon built with protocol buffers
as a first-class citizen.

Cascade aims to be a Kafka-like tool for the enthusiast developer, without the investment of
running Zookeeper and Kafka on a single node and using as few resources as possible. The JVM
is a remarkably powerful platform, but tends to use more steady-state resources (CPU, memory)
for low-traffic projects.
...
Cascade takes some of the best elements of Kafka and scales them down. Topics do not have partitions, you
probably shouldn't be pushing Cascade so hard as to need partitioned behavior. At the same time, 
it's quite nice to have monotonically increasing message identifiers and write-ordered delivery.
Cascade eschews Kafka's custom binary protocol (efficient as it may be) and implements a more standard
gRPC API and while retaining a HTTP/JSON interface for non-RPC clients.

Cascade was built as a way to have Kafka-like semantics for a crawl/transform/notify infrastructure
...
Additionally, Cascade powers a custom analytics stack.

## Why should I use Cascade?
...
created as each segment becomes too old or too big. Cascade attempts to retain data no longer than
...
Cascade provides some durability guarantees. Upon receipt of an successful AppendLogs response, the data
in the request has been flushed to disk. However, it is possible that a large batch of messages may
trigger an early flush of data to disk. If Cascade were to exit before a final flush, it may lead to some
...
Cascade may opt to implement transactional semantics in the future (prepare, append, {commit/abort}) to provide
...
1) Offset. This is assigned by Cascade at publish time, a monotonically increasing 64-bit integer.
This represents the order the message has in the topic. Offset 0 is the first message, offset 1 the
second and so on.

2) Timestamp. This is assigned by Cascade at publish time, also monotonically increasing. That is,
...
4) Annotations. This is a map of string to string KV pairs. These can be used to have Cascade filter
...
Like Kafka, Cascade defaults to a 7 day retention period on log segments. This retention period is a
...
Unlike Kafka, Cascade has two modes of operation: poll-based and stream-based. Poll-based operation 
allows a client to ask for up to N messages starting at some offset X. This is how Kafka works. 
Cascade also provides a stream-based API in which you can ask for messages starting at some offset X
(or latest) and Cascade will maintain an open connection with your client, pushing messages as they
...
state with Cascade. This cursor allows for stateful iteration and low-latency pubsub semantics.
...
Cascade is primarily distributed under the terms of both the MIT license and the Apache License (Version 2.0), 
